  ...
  <div class="fill_bg">
    ...
    <div class="fill_top">
      <div class="qu-wrap" v-if="!isLoading">
        ...
        <footer>
          <a-button class="btn" type="primary" v-if="isFillMode" @click="iterator = submit(); iterator.next()">提交问卷</a-button>
          <a-button class="btn" v-else @click="back()">返回</a-button>
        </footer>
        <div class="fill_bottom" >
          <span style="color:#333;">北京阳光印易科技有限公司</span>&nbsp;&nbsp;&nbsp;&nbsp;提供技术支持
        </div>
      </div>
    </div>
    <a-modal
...
export default {
  ...
  created() {
    ...
    this.isFillMode = this.$route.path === "/lists/fill";
    this._getPaper();
  },
  ...
  methods: {
    ...
    *commitSuccess() {
      ...
      this.$router.push({ path: "/lists/success" });
...
.fill_bg {
  background: #f3f6fa url(./img/fill_bg.jpg) center top repeat-x;
  min-height: 100vh;
}
.fill_bottom{
  text-align: center;
